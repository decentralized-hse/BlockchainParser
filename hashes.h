...
#error "SHA256_UNROLL must be 1, 2, 4, 8, 16, 32, or 64!"
    ...
    void doubleSHA256(void* input, uint32_t size, uint8_t destHash[32]) {
        uint8_t tmp[32];
        BLOCKCHAIN_SHA256::computeSHA256(input, size, tmp);
        BLOCKCHAIN_SHA256::computeSHA256(tmp, 32, destHash);
    }
...
namespace BLOCKCHAIN_BITCOIN_ADDRESS
{
    ...
    void bitcoinRIPEMD160ToAddress(const uint8_t ripeMD160[20], uint8_t version,uint8_t output[25])
    {
        uint8_t hash1[32]; // holds the intermediate SHA256 hash computations
        output[0] = version;	// Store a network byte of 0 (i.e. 'main' network)
...
/*
void printOutputAddress(const Output& output) {
    char scratch[256];
    bool ok=true;
